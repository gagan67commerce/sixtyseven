 ...
 */
namespace SixtySeven\Faq\Model\ResourceModel;
...
use SixtySeven\Faq\Model\Config\Source\Urlkey;
...
class Faq extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    const TBL_ATT_PRODUCT = 'sixtyseven_faq_attachment_rel';
    ...
    {
        $this->_init('sixtyseven_faq', 'faq_id');
    }
    ...
    protected function _afterLoad(AbstractModel $object)
    {
        $faq_store = $this->getConnection()
            ->select()
            ->from($this->getTable('sixtyseven_faq_store'), ['store_id'])
            ->where('faq_id = :faq_id');
        ...
        $faq_category = $this->getConnection()
            ->select()
            ->from($this->getTable('sixtyseven_faq_category_id'), ['category_id'])
            ->where('faq_id = :faq_id');
    ...
    protected function duplicateFaqIdentifier(AbstractModel $object)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->join(
                ['faq_store' => $this->getTable('sixtyseven_faq_store')],
    ...
    public function getFaqCategory($faq_id = null)
    {
        $select = $this->getConnection()->select()
            ...
            ->joinLeft(
                ['faqcat' => $this->getTable('sixtyseven_faq_category_id')],
                'faq.faq_id = faqcat.faq_id',
                ['category_id']
            )
            ->joinLeft(
                ['cat' => $this->getTable('sixtyseven_faq_category')],
                'faqcat.category_id = cat.category_id',
                ['title', 'identifier']
            )
            ->joinLeft(
                ['cat_store' => $this->getTable('sixtyseven_faq_category_store')],
    ...
    public function updateNumberOfFaqsInCategory()
    {
        ...
        $select = $adapter->select()->from($this->getTable('sixtyseven_faq_category_id'), ['count' => 'COUNT(category_id)', 'category_id'])->group('category_id');

        $faq_category_results = $this->getConnection()->fetchAll($select);

        $adapter->update($this->getTable('sixtyseven_faq_category'), ['count' => '0']);

        foreach ($faq_category_results as $value) {
            $adapter->update($this->getTable('sixtyseven_faq_category'), ['count' => $value['count']], ['category_id = ?' => (int) $value['category_id']]);
        };
    ...
    protected function saveFaqRelation(AbstractModel $object)
    {
        ...
        if ($faq_id && (int) $faq_id > 0) {
            ...
            if ($category_id) {
                ...
                $adapter->delete($this->getTable('sixtyseven_faq_category_id'), $condition);

                $faq_category = [
                    'faq_id' => (int) $faq_id,
                    'category_id' => (int) $category_id
                ];
                $adapter->insertMultiple($this->getTable('sixtyseven_faq_category_id'), $faq_category);
            ...
            if ($stores) {
                $condition = ['faq_id = ?' => (int) $faq_id];
                $adapter->delete($this->getTable('sixtyseven_faq_store'), $condition);
                ...
                $adapter->insertMultiple($this->getTable('sixtyseven_faq_store'), $data);
